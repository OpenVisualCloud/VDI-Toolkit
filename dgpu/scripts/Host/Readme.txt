...
usage: ./6_VM-vgpu-unplug.sh $VM_NAME $target_XML
e.g. ./6_VM-vgpu-unplug.sh win2k19 device.xml //this command will detach vGPU from VM named win2k19 and move the vGPU HW data config into a new file device.xml.
					      //if we need to retach the vGPU, we could use virsh command "virsh attach-device win2k19 device.xml"